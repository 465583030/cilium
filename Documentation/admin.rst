...
optional but allows the Cilium agent to pin BPF resources to a filesystem which
makes these resources persistent across agent restarts. If the BPF filesystem
is not mounted in the host filesystem then all BPF resources created in the
namespace of the pod will be destroyed when the pod is stopped and thus network
connectivity of all local pods will be removed. By mounting the BPF filesystem
in the host mount namespace, the agent can pin BPF resources in a persistent
manner which allows to restart the Cilium agent without affecting networking
connectivity of pods.
...
If you are using systemd to manage your kubelet, the easiest way to achieve
this is to add an ``ExecPre`` line to your ``/etc/systemd/kubelet.service``
file like this:
...
configuration if default behaviour is not desirable:
...
kubectl will start deploying the daemonset, when retrieving the DaemonSet
status you will see a desired number Cilium pods which should match the
numberof nodes you are running:
